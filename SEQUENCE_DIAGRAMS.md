...
```mermaid
sequenceDiagram
    title ChilizTV — Full System Deployment
    ...
    rect rgb(230, 255, 230)
        ...
        Deployer->>Escrow: deploy PayoutEscrow(usdt, safeAddress)
        activate Escrow
        Note right of Escrow: Ownable(_owner = Safe)<br/>usdt = immutable USDT<br/>ReentrancyGuard + Pausable
        Escrow-->>Deployer: Escrow deployed ✓
        deactivate Escrow
    end
    ...
    rect rgb(255, 240, 220)
        ...
        Deployer->>SWFactory: deploy StreamWalletFactory(<br/>  initialOwner, treasury, feeBps,<br/>  kayenRouter, fanToken, usdt)
        activate SWFactory
        ...
        SWFactory-->>Deployer: StreamWalletFactory deployed ✓
    ...
    rect rgb(255, 230, 230)
        ...
        Deployer->>SwapRouter: deploy ChilizSwapRouter(<br/>  masterRouter, tokenRouter,<br/>  usdt, wchz, treasury, feeBps)
        activate SwapRouter
        Note right of SwapRouter: Immutables: masterRouter, tokenRouter, usdt, wchz<br/>Mutable: treasury, platformFeeBps<br/>Ownable(msg.sender) + ReentrancyGuard
        SwapRouter-->>Deployer: SwapRouter deployed ✓
    ...
    rect rgb(240, 240, 255)
        ...
        Safe->>USDT: approve(Escrow, amount)
        Safe->>Escrow: fund(amount)
        Escrow-->>Safe: emit Funded ✓
    end
...
```mermaid
sequenceDiagram
    title Betting System — Happy Path (End-to-End)
    ...
    rect rgb(200, 220, 255)
        ...
        Admin->>Factory: createFootballMatch("Real Madrid vs Barça", owner)
        activate Factory
        Factory->>Proxy: new ERC1967Proxy(FOOTBALL_IMPL, initData)
        activate Proxy
        Proxy->>FImpl: delegatecall initialize("Real Madrid vs Barça", owner)
        Note right of FImpl: __BettingMatchV2_init:<br/>Grant owner ALL roles:<br/>DEFAULT_ADMIN, ADMIN, RESOLVER,<br/>PAUSER, TREASURY, ODDS_SETTER<br/>matchName = "Real Madrid vs Barça"<br/>sportType = "FOOTBALL"
    ...
    rect rgb(230, 255, 230)
        ...
        Admin->>Proxy: setUSDTToken(usdtAddress)
        Note right of Proxy: onlyRole(ADMIN_ROLE)<br/>emit USDTTokenSet
        ...
        Note right of Proxy: onlyRole(DEFAULT_ADMIN_ROLE)<br/>Allows ChilizSwapRouter to call placeBetUSDTFor
    ...
    rect rgb(255, 245, 220)
        ...
        activate Proxy
        Note right of Proxy: onlyRole(ADMIN_ROLE)<br/>_validateOdds(22000) ✓ [10001..1000000]<br/>maxSelections = 2 (Home/Draw/Away)<br/>State = Inactive<br/>oddsRegistry[0] = [22000], currentIndex = 1
        ...
        Note right of Proxy: State: Inactive → Open<br/>emit MarketStateChanged(0, Inactive, Open)

        Admin->>Proxy: openMarket(1)
        Note right of Proxy: State: Inactive → Open

        Treasury->>Proxy: fundUSDTTreasury(50000e6)
        Note right of Proxy: onlyRole(TREASURY_ROLE)<br/>safeTransferFrom(treasury, proxy, 50000 USDT)
    end

    rect rgb(255, 240, 220)
        Note over Admin,Escrow: STEP 4: USERS PLACE BETS

        User1->>USDT: approve(proxy, 500e6)
        User1->>Proxy: placeBetUSDT(marketId=0, selection=0, 500e6)
        activate Proxy
        Note right of Proxy: Checks: Open ✓, !Paused ✓, USDT set ✓<br/>_validateSelection(0, 0): 0 ≤ 2 ✓<br/>odds = 22000 (2.20x)<br/>potentialPayout = 500 × 22000 / 10000 = 1100 USDT<br/>Solvency: liabilities + 1100 ≤ balance + 500 ✓<br/>safeTransferFrom(user1, proxy, 500 USDT)
        Proxy-->>User1: emit BetPlaced(0, user1, idx=0, 500e6, sel=0, 22000, oddsIdx=1)
        deactivate Proxy

        OddsSetter->>Proxy: setMarketOdds(0, 18000)
        Note right of Proxy: onlyRole(ODDS_SETTER_ROLE)<br/>Market Open ✓<br/>oddsRegistry[0] = [22000, 18000]<br/>currentIndex = 2<br/>emit OddsUpdated(0, 22000, 18000, 2)

        User2->>USDT: approve(proxy, 300e6)
        User2->>Proxy: placeBetUSDT(marketId=0, selection=2, 300e6)
        activate Proxy
        Note right of Proxy: User2 gets NEW odds: 18000 (1.80x)<br/>potentialPayout = 300 × 18000 / 10000 = 540 USDT<br/>Bet stored with oddsIndex=2
    ...
    rect rgb(220, 255, 220)
        ...
        Note right of Proxy: Open → Suspended (match kicked off)<br/>No more bets accepted

        Admin->>Proxy: closeMarket(0)
        Note right of Proxy: Suspended → Closed (match ended)<br/>Awaiting result
    end
    ...
    rect rgb(255, 230, 230)
        ...
        activate Proxy
        Note right of Proxy: onlyRole(RESOLVER_ROLE)<br/>State must be Closed or Open<br/>core.result = 0 (Home wins)<br/>core.resolvedAt = block.timestamp<br/>State: Closed → Resolved
    ...
    rect rgb(240, 240, 255)
        ...
        activate Proxy
        Note right of Proxy: nonReentrant, Resolved ✓, !Paused ✓<br/>bet.selection(0) == core.result(0) ✓ WINNER<br/>betOdds = oddsRegistry[oddsIndex=1-1] = 22000<br/>payout = 500 × 22000 / 10000 = 1100 USDT<br/>bet.claimed = true (CEI)<br/>totalUSDTLiabilities -= 1100<br/>_disburse(user1, 1100): balance ≥ 1100 ✓
        Proxy->>USDT: safeTransfer(user1, 1100e6)
        USDT-->>User1: 1100 USDT received ✓
...
```mermaid
sequenceDiagram
    title Betting System — All Failure Routes
    ...
    rect rgb(255, 220, 220)
        Note over User,Escrow: ❌ BET PLACEMENT FAILURES

        User->>Proxy: placeBetUSDT(99, 0, 100e6)
        Proxy-->>User: REVERT InvalidMarketId(99)<br/>marketId >= marketCount

        User->>Proxy: placeBetUSDT(0, 0, 100e6) [market Inactive]
        Proxy-->>User: REVERT InvalidMarketState(0, Inactive, Open)<br/>Market not Open

        User->>Proxy: placeBetUSDT(0, 0, 100e6) [paused]
        Proxy-->>User: REVERT EnforcedPause<br/>Contract is paused

        User->>Proxy: placeBetUSDT(0, 0, 0)
        Proxy-->>User: REVERT ZeroBetAmount<br/>amount == 0

        User->>Proxy: placeBetUSDT(0, 0, 100e6) [no USDT set]
        Proxy-->>User: REVERT USDTNotConfigured<br/>usdtToken == address(0)

        User->>Proxy: placeBetUSDT(0, 0, 100e6) [no odds set]
        Proxy-->>User: REVERT OddsNotSet(0)<br/>currentIndex == 0

        User->>Proxy: placeBetUSDT(0, 5, 100e6) [football WINNER]
        Proxy-->>User: REVERT InvalidSelection(0, 5, 2)<br/>selection > maxSelections

        User->>Proxy: placeBetUSDT(0, 0, 1000000e6) [huge bet]
        Proxy-->>User: REVERT USDTSolvencyExceeded<br/>liabilities + potentialPayout > balance + deposit

        User->>Proxy: placeBetUSDT(0, 0, 100e6) [no USDT approval]
        Proxy-->>User: REVERT SafeERC20: low-level call failed<br/>ERC20 transferFrom fails
    end

    rect rgb(255, 235, 220)
        Note over User,Escrow: ❌ CLAIM / REFUND FAILURES
        ...
        Proxy-->>User: REVERT InsufficientUSDTBalance(1100, 200)<br/>balance < payout and no escrow set
    ...
    rect rgb(255, 245, 220)
        Note over User,Escrow: ❌ ADMIN / ROLE FAILURES
        ...
        BadActor->>Proxy: placeBetUSDTFor(user, 0, 0, 100e6)
        ...
        Admin->>Proxy: emergencyWithdrawUSDT(100e6) [not paused]
...
```mermaid
sequenceDiagram
    title PayoutEscrow — Happy Path + Failures
    ...
    rect rgb(230, 255, 230)
        Note over Safe,Escrow: ✅ HAPPY: Direct Payout (no escrow needed)

        Winner->>Proxy: claim(marketId, betIndex)
        Proxy->>Proxy: _disburse(winner, 1100 USDT)
        Proxy->>Proxy: contractBalance(1500) >= 1100 ✓
        Proxy->>USDT: safeTransfer(winner, 1100)
        USDT-->>Winner: 1100 USDT ✓
    end

    rect rgb(220, 240, 255)
        Note over Safe,Escrow: ✅ HAPPY: Escrow Fallback Payout
        ...
        activate Escrow
        Note right of Escrow: onlyAuthorizedMatch ✓<br/>whenNotPaused ✓<br/>balance(5000) >= 700 ✓<br/>totalDisbursed += 700<br/>disbursedPerMatch[proxy] += 700
        Escrow->>USDT: safeTransfer(proxy, 700)
        deactivate Escrow
    ...
    rect rgb(255, 220, 220)
        Note over Safe,Escrow: ❌ FAILURE: Escrow Not Set

        Winner->>Proxy: claim(marketId, betIndex)
        Proxy->>Proxy: contractBalance(400) < 1100
        Note right of Proxy: payoutEscrow == address(0)
        Proxy-->>Winner: REVERT InsufficientUSDTBalance(1100, 400)
    end

    rect rgb(255, 230, 220)
        Note over Safe,Escrow: ❌ FAILURE: Escrow Underfunded
    ...
    rect rgb(255, 240, 220)
        Note over Safe,Escrow: ❌ FAILURE: Match Not Authorized on Escrow
    ...
    rect rgb(255, 245, 230)
        Note over Safe,Escrow: ❌ FAILURE: Escrow Paused
...
All three donation paths: CHZ, ERC20 token, and USDT direct.

```mermaid
sequenceDiagram
    title Streaming Donations — Happy Path (All Payment Paths)
    ...
    rect rgb(200, 220, 255)
        ...
        activate SwapRouter
        Note right of SwapRouter: Checks: value > 0 ✓, streamer ≠ 0 ✓<br/>block.timestamp ≤ deadline ✓

        SwapRouter->>Kayen: swapExactETHForTokens{50 CHZ}(<br/>  minOut, [WCHZ, USDT], router, deadline)
        Kayen-->>SwapRouter: receive 120 USDT

        SwapRouter->>SwapRouter: _splitAndTransfer(streamer, 120)
        Note right of SwapRouter: fee = 120 × 500 / 10000 = 6 USDT<br/>streamerAmt = 120 - 6 = 114 USDT

        SwapRouter->>USDT: safeTransfer(treasury, 6)
        USDT-->>Treasury: 6 USDT fee

        SwapRouter->>USDT: safeTransfer(streamer, 114)
        USDT-->>Streamer: 114 USDT
        ...
        activate Wallet
        Note right of Wallet: onlyAuthorized (swapRouter) ✓<br/>lifetimeDonations[donor] += 120<br/>totalRevenue += 120
        Wallet-->>SwapRouter: emit DonationReceived ✓
        deactivate Wallet

        SwapRouter-->>Donor: emit DonationWithCHZ ✓
        deactivate SwapRouter
    end
    ...
    rect rgb(230, 255, 230)
        ...
        activate SwapRouter
        Note right of SwapRouter: Checks: amount > 0, token ≠ 0, streamer ≠ 0<br/>token ≠ usdt ✓, deadline ✓

        SwapRouter->>USDT: safeTransferFrom(donor, router, 1000 tokens)
        SwapRouter->>Kayen: swapExactTokensForTokens(<br/>  1000, minOut, [token, USDT], router, deadline)
        Kayen-->>SwapRouter: receive 80 USDT

        SwapRouter->>SwapRouter: _splitAndTransfer(streamer, 80)
        SwapRouter->>USDT: safeTransfer(treasury, 4)
        SwapRouter->>USDT: safeTransfer(streamer, 76)
        SwapRouter->>Wallet: recordDonationByRouter(donor, 80, 4, 76, "msg")
        SwapRouter-->>Donor: emit DonationWithToken ✓
        deactivate SwapRouter
    end

    rect rgb(255, 240, 220)
        Note over Donor,Treasury: PATH C: donateWithUSDT (direct, no swap)

        Donor->>SwapRouter: donateWithUSDT(streamer, "thanks!", 50e6)
        activate SwapRouter
        Note right of SwapRouter: No Kayen swap needed

        SwapRouter->>USDT: safeTransferFrom(donor, router, 50)
        SwapRouter->>USDT: safeTransfer(treasury, 2.5)
        SwapRouter->>USDT: safeTransfer(streamer, 47.5)
        SwapRouter->>Wallet: recordDonationByRouter(donor, 50, 2.5, 47.5, "thanks!")
        SwapRouter-->>Donor: emit DonationWithUSDTEvent ✓
        deactivate SwapRouter
    end
    ...
    rect rgb(240, 240, 240)
        ...
        activate SWFactory
        SWFactory->>USDT: transferFrom(donor, factory, 500 fan tokens)
        ...
        activate Wallet
        Note right of Wallet: Pull fan tokens from factory<br/>fee = 500 × 5% = 25 tokens<br/>Swap 25 tokens → USDT → treasury<br/>Swap 475 tokens → USDT → streamer
        Wallet-->>SWFactory: emit DonationReceived ✓
        deactivate Wallet
        SWFactory-->>Donor: emit DonationProcessed ✓
        deactivate SWFactory
    end
```
...
All subscription paths: CHZ, ERC20, and USDT direct.

```mermaid
sequenceDiagram
    title Streaming Subscriptions — Happy Path
    ...
    rect rgb(200, 220, 255)
        ...
        activate SwapRouter
        Note right of SwapRouter: Checks: value > 0, streamer ≠ 0,<br/>duration > 0, deadline ✓

        SwapRouter->>Kayen: swapExactETHForTokens{10 CHZ}(...)
        Kayen-->>SwapRouter: 25 USDT

        SwapRouter->>SwapRouter: _splitAndTransfer(streamer, 25)
        Note right of SwapRouter: fee = 1.25, streamer = 23.75

        SwapRouter->>USDT: safeTransfer(treasury, 1.25)
        SwapRouter->>USDT: safeTransfer(streamer, 23.75)
        ...
        activate Wallet
        Note right of Wallet: onlyAuthorized ✓<br/>sub.active? extend expiry : new sub<br/>sub.amount += 25<br/>sub.expiryTime = now + 30d (or extend)<br/>totalSubscribers++ (if new)<br/>totalRevenue += 25
        Wallet-->>SwapRouter: emit SubscriptionRecorded ✓
        deactivate Wallet

        SwapRouter-->>Sub: emit SubscriptionWithCHZ ✓
        deactivate SwapRouter
    end
    ...
    rect rgb(230, 255, 230)
        ...
        activate SwapRouter
        SwapRouter->>Kayen: swap 500 tokens → USDT
        Kayen-->>SwapRouter: 40 USDT
        SwapRouter->>USDT: fee → treasury, rest → streamer
        SwapRouter->>Wallet: recordSubscriptionByRouter(sub, 40, 30d)
        SwapRouter-->>Sub: emit SubscriptionWithToken ✓
        deactivate SwapRouter
    end

    rect rgb(255, 240, 220)
        Note over Sub,Treasury: PATH C: subscribeWithUSDT (direct)

        Sub->>SwapRouter: subscribeWithUSDT(streamer, 30d, 100e6)
        activate SwapRouter
        SwapRouter->>USDT: safeTransferFrom(sub, router, 100)
        SwapRouter->>USDT: safeTransfer(treasury, 5)
        SwapRouter->>USDT: safeTransfer(streamer, 95)
        SwapRouter->>Wallet: recordSubscriptionByRouter(sub, 100, 30d)
        SwapRouter-->>Sub: emit SubscriptionWithUSDTEvent ✓
        deactivate SwapRouter
    end
    ...
    rect rgb(240, 240, 255)
        ...
        SwapRouter-->>Sub: Subscription extended ✓
        deactivate SwapRouter
    end
    ...
    rect rgb(240, 240, 240)
        ...
        activate Wallet
        Note right of Wallet: onlyStreamer ✓<br/>available = USDT.balanceOf(wallet)<br/>100 ≤ available ✓<br/>totalWithdrawn += 100<br/>safeTransfer(streamer, 100)
        Wallet-->>Streamer: emit RevenueWithdrawn(streamer, 100) ✓
        deactivate Wallet
    end
```
...
```mermaid
sequenceDiagram
    title Streaming System — All Failure Routes
    ...
    rect rgb(255, 220, 220)
        Note over User,Wallet: ❌ SWAP ROUTER — DONATION FAILURES
        ...
        User->>SwapRouter: donateWithToken(usdtAddr, 100, str, msg, min, dl)
        SwapRouter-->>User: REVERT TokenIsUSDT<br/>Use donateWithUSDT instead

        User->>SwapRouter: donateWithUSDT(streamer, msg, 0)
        SwapRouter-->>User: REVERT ZeroValue

        User->>SwapRouter: donateWithCHZ{1 CHZ}(str, msg, 999999e6, dl)
        SwapRouter->>Kayen: swap → only 2 USDT received
        Kayen-->>SwapRouter: REVERT amountOutMin not met<br/>(Kayen slippage protection)
        SwapRouter-->>User: REVERT (swap failed)
    end

    rect rgb(255, 230, 220)
        Note over User,Wallet: ❌ SWAP ROUTER — SUBSCRIPTION FAILURES
    ...
    rect rgb(255, 235, 220)
        Note over User,Wallet: ❌ SWAP ROUTER — BETTING FAILURES

        User->>SwapRouter: placeBetWithCHZ{0}(match, 0, 0, min, dl)
        SwapRouter-->>User: REVERT ZeroValue

        User->>SwapRouter: placeBetWithUSDT(address(0), 0, 0, 100)
        SwapRouter-->>User: REVERT ZeroAddress

        User->>SwapRouter: placeBetWithToken(usdt, 100, match, 0, 0, min, dl)
        SwapRouter-->>User: REVERT TokenIsUSDT
    end

    rect rgb(255, 240, 220)
        Note over User,Wallet: ❌ FACTORY FAILURES
    ...
    rect rgb(255, 245, 220)
        Note over User,Wallet: ❌ STREAM WALLET FAILURES
        ...
        Wallet-->>User: REVERT InsufficientBalance<br/>amount > USDT.balanceOf(wallet)
...
```mermaid
sequenceDiagram
    title ChilizSwapRouter — Complete Payment Flow Map
    ...
    rect rgb(200, 220, 255)
        Note over User,Streamer: 🏆 BETTING PATHS

        Note over User: placeBetWithCHZ{CHZ}(match, mkt, sel, min, dl)
        User->>SwapRouter: CHZ (native)
        SwapRouter->>Kayen: CHZ → USDT (masterRouter.swapExactETHForTokens)
        SwapRouter->>Match: safeTransfer(USDT) + placeBetUSDTFor(user)

        Note over User: placeBetWithToken(token, amt, match, mkt, sel, min, dl)
        User->>SwapRouter: ERC20 token
        SwapRouter->>Kayen: ERC20 → USDT (tokenRouter.swapExactTokensForTokens)
        SwapRouter->>Match: safeTransfer(USDT) + placeBetUSDTFor(user)

        Note over User: placeBetWithUSDT(match, mkt, sel, amt)
        User->>SwapRouter: USDT direct
        SwapRouter->>Match: safeTransfer(USDT) + placeBetUSDTFor(user)
        Note right of Match: NO swap needed
    end

    rect rgb(230, 255, 230)
        Note over User,Streamer: 🎁 DONATION PATHS

        Note over User: donateWithCHZ{CHZ}(streamer, msg, min, dl)
        User->>SwapRouter: CHZ
        SwapRouter->>Kayen: CHZ → USDT
        ...
        SwapRouter->>Kayen: ERC20 → USDT
        SwapRouter->>Treasury: fee
        SwapRouter->>Streamer: remainder
        SwapRouter->>Wallet: recordDonationByRouter()

        Note over User: donateWithUSDT(streamer, msg, amt)
        User->>SwapRouter: USDT
        SwapRouter->>Treasury: fee
        SwapRouter->>Streamer: remainder
        SwapRouter->>Wallet: recordDonationByRouter()
    end
    ...
    rect rgb(255, 240, 220)
        Note over User,Streamer: 📺 SUBSCRIPTION PATHS
        ...
        Note over User: subscribeWithUSDT(streamer, duration, amt)
        User->>SwapRouter: USDT
        SwapRouter->>Treasury: fee
        SwapRouter->>Streamer: remainder
        SwapRouter->>Wallet: recordSubscriptionByRouter()
...
| `USDTNotConfigured` | BettingMatch | usdtToken not set |
| `OddsNotSet` | BettingMatch | No odds registered |
| `InvalidSelection` | Football/Basketball | selection > maxSelections |
| `USDTSolvencyExceeded` | BettingMatch | Liabilities exceed balance |
...
| `TokenIsUSDT` | ChilizSwapRouter | Use direct USDT function |
